...
if [ ! -e $DATA_DIRECTORY/toran/composer/auth.json ]; then
    if [ "${TORAN_TOKEN_GITHUB}" != "false" ]; then
        ...
        echo "  You need to setup a GitHub OAuth token. Toran Proxy makes quite a few requests and will use up the API calls limit if it is left unauthenticated."
        echo "  Head to https://github.com/settings/tokens/new to create a new token or use an existing one. You need to select the public_repo credentials under repo. Select the entire repo permissions if you are going to use private repositories from GitHub with Toran Proxy."
    fi
else
  if [ "${TORAN_TOKEN_GITHUB}" != "false" ]; then
      echo "Updating Github OAuth Token..."
      sed -i "s|\"github.com\":.*|\"github.com\":\"$TORAN_TOKEN_GITHUB\"|g" $DATA_DIRECTORY/toran/composer/auth.json
  fi
fi
...
if [ ! -d $DATA_DIRECTORY/mirrors ]; then
    echo "Creating directory mirrors..."
